    ...
    return new Proxy(this, {
      get: (target, prop) => {
        if (prop in target) {
          ...
          typeof prop === "string" &&
          isDataProperty(prop, this.context, this._dataPrefix)
        ) {
  ...
  _handleAttributeChange(mutations) {
    for (const mutation of mutations) {
      if (
        ...
        mutation.attributeName?.startsWith(this._dataPrefix)
      ) {
  ...
  _handleComponentAttributeChange(mutation, target) {
    for (const [componentName, component] of this._components.entries()) {
      if (component.context === target) {
        const prop = attributeToProperty(
        ...
        if (
          typeof this[`${componentName}${capitalize(prop)}Changed`] ===
          "function"
        ) {
          this[`${componentName}${capitalize(prop)}Changed`](
            newValue,
            mutation.oldValue
          );
        }
